    ...
    for t in T_d:
        for s in S:
            if (s, t) not in V_nd[t]:
                if t not in s_by_t.keys():
                    s_by_t[t] = [((s, t), q[(s, 0), t])] # add group k for time t
                else:
                    s_by_t[t].append(((s, t), q[(s, 0), t])) # add group k for time t          
    for t in s_by_t.keys():
        s_by_t[t] = sorted(s_by_t[t], key = lambda x: x[1], reverse = True)
    
    """ extract the most 'valuable' s """
    largest_num = 6
    int_s_by_t = {}
    for t in s_by_t.keys():
...
# =============================================================================
    ...
    print('********** Z take nonzero integer value **********')
    # Z = best_ub_Z.copy()
    
# =============================================================================
#     for sub in sub_Z:
#         if best_ub_Z[sub] != 0 and int(best_ub_Z[sub]) == best_ub_Z[sub]:
#             print(sub, best_ub_Z[sub])
# =============================================================================

# =============================================================================
#     print('********** Z take fractional value **********')
#     for sub in sub_Z:
#         if int(Z[sub]) != Z[sub].X:
#             print(sub, Z[sub].X)
# =============================================================================
    print('total iteration', counter)
    print('best ub iteration', counter_ub)
    print('best lb iteration', counter_lb)    

    print("********** fixing those Z that takes integer values and forcing those taking non-integer as integer")
# =============================================================================
#     ls_m = m.copy()
#     m = ls_m.copy()
# =============================================================================
    ...
    for sub in sub_Z:
        if int(best_ub_Z[sub]) == best_ub_Z[sub]:
            Z[sub].ub = best_ub_Z[sub]
            Z[sub].lb = best_ub_Z[sub]
        else:
            Z[sub].setAttr(GRB.Attr.VType, GRB.INTEGER)
            
# =============================================================================
#     for sub in sub_Z:
#         if int(best_lb_Z[sub]) == best_lb_Z[sub]:
#             Z[sub].ub = best_lb_Z[sub]
#             Z[sub].lb = best_lb_Z[sub]
#         else:
#             Z[sub].setAttr(GRB.Attr.VType, GRB.INTEGER)
# =============================================================================
    ...
    print('********* restored ub *********')
    print(m.objVal)
    time_log[ending_time] = {'ub': Xi_ub, 'lb':Xi_lb, 're_ub': m.objVal,
                             'total_iter': counter, 'ub_iter': counter_ub,
                             'lb_iter': counter_lb}

print(time_log)
with open('321_OA_B_SCA', 'w') as log_result:
