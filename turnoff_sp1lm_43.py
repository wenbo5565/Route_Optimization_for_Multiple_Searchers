...
grid_size = 9
ending_time_grid = [10, 12, 14] # , 12, 14, 15, 16, 17, 18, 20]
J = 2
J_2 = int(J * 0.7)
J_1 = J - J_2
...
for ending_time in ending_time_grid:
    ...
    sub_V = []
    for l in L:
        sub_V = sub_V + list(product([l], S_expand, T, J_ind[l]))
    sub_Q = []
    for l in L:
        sub_Q = sub_Q + list(product([l], S_C, T, J_ind[l]))
    ...
    """
    """ 4.3b """    
    m.addConstrs((Q[l, s_c, t, j] <= q[s_c, t] * (1 - np.exp(-j * alpha[l, s_c[1]])) * V[l, s_c[0], t, j] for l in L for s_c in S_C for t in T for j in J_ind[l]), name = '43b')

    """ 4.3c """
    m.addConstrs((Q[l, s_c, t, j] <= (1 - np.exp(-j * alpha[l, s_c[1]])) * P[s_c, t] for l, s_c, t, j in sub_Q), name = '43c')
    ...
    """ 4.3f """
    m.addConstrs((W[s_c, t] <= np.exp(-j * alpha[l, s_c[1]]) * P[s_c, t] + q[s_c, t] * (1 - np.exp(-j * alpha[l, s_c[1]])) * (1 - V[l, s_c[0], t, j]) for l in L for s_c in S_C for t in T for j in J_ind[l]), name = '43f')
    ...
    m.addConstrs((sum(X[l, s_expand_prime, s_expand, t - 1] for s_expand_prime in S_expand if is_backward_cell(s_expand_prime, s_expand, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end)) 
                  == sum(j * V[l, s_expand, t, j] for j in J_ind[l]) for l in L for s_expand in S_expand for t in T), name = '43i')
    """ 4.3j """
    m.addConstrs((sum(V[l, s_expand, t, j] for j in J_ind[l]) <= 1 for l in L for s_expand in S_expand for t in T), name = '43j')
