...
# =============================================================================
    ...
    group_by_t_w_q = {} # key is t; value is list of cell and its q
    for k_t in group_cnt.keys():
        if k_t[1] not in group_by_t_w_q.keys():
            group_by_t_w_q[k_t[1]] = [(k_t[0], q[k_t[0], k_t[1]])] # add group k for time t
        else:
            group_by_t_w_q[k_t[1]].append((k_t[0], q[k_t[0], k_t[1]])) # add group k for time t
    ...
    group_by_t_largest = {}
    for t in group_by_t_w_q.keys():
        group_by_t_largest[t] = group_by_t_w_q[t][:largest_num]
        group_by_t_largest[t] = [c[0] for c in group_by_t_largest[t]]
    ...
    for group in group_cnt.keys():
        c, t = group
        if c in group_by_t_largest[t]:
            ZZZ[group] = m.addVar(lb = 0, ub = J, vtype = GRB.INTEGER)
        else:
            ZZZ[group] = m.addVar(lb = 0, ub = J, vtype = GRB.CONTINUOUS)
...
print(time_log)
with open('OA_grouping_time_log_T8_J15.txt', 'w') as log_result:
