...
constr49c = m.addConstrs((sum(X[l, s_prime, s, t - 1] for s_prime in S_expand if is_backward_cell(s_prime, s, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end))
              == ZZZ[l, s, t] for l in L for s in S_expand for t in T if t not in T_no_detect and (s, t) not in part_T_no_detect_pairs), name = 'constr49c') #2d

# =============================================================================
#     for constr in x_zzz_link.values():
...
part_Z_0 = m.addVars(list(product(L, part_detect_T)), name = 'part_Z_0', vtype = GRB.CONTINUOUS)
constr49d = {}
for l in L:
    for t_loop in part_detect_T:
    # print(t_loop)
        constr49d[l, t_loop] = m.addConstr((sum(X[l, s_prime, s, t_loop - 1] for s, t in part_T_no_detect[t_loop] for s_prime in S_expand if searcher_reachable[s, t_loop] > 0 and is_backward_cell(s_prime, s, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end)) == part_Z_0[l, t_loop]), name = 'constr49d_' + str(l) + '_' + str(t_loop)) 


constr49e = {}
for l in L:
    for t in T_no_detect:
        constr49e[l, t] = m.addConstr((sum(X[l, s_prime, s, t - 1] for s in S_expand for s_prime in S_expand if is_backward_cell(s_prime, s, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end)) == n_L[l]), name = 'constr49e_' + str(l) + str(t)) 
...
#         constr49f[l, t] = m.addConstr((part_Z_0[l, t] + sum(ZZZ[l, s, t] for l, s, t_inner in sub_ZZZ if t_inner == t) == n_L[l]), name = 'constr49f_' + str(l) + '_' + str(t))
# 
# =============================================================================
# =============================================================================
#         for l in L:
...
"""
# 2.4b
constrXcontinum = m.addConstrs((sum(X[l, s_prime, s, t - 1] for s_prime in S_expand if is_backward_cell(s_prime, s, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end))
...
"""
constrInitX = m.addConstrs((sum(X[l, s, s_prime, 0] for s_prime in S_expand if is_forward_cell(s, s_prime, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end))
...
""" 2.4d """
constrNst = m.addConstrs((sum(X[l, s_expand_prime, s_expand, t - 1] for s_expand_prime in S_expand for l in L if is_backward_cell(s_expand_prime, s_expand, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end))
...
"""
constrOltEqual = m.addConstrs((sum(X[l, s, s_prime, t - 1] for s in [s_init] for s_prime in S_expand if s_prime != s and is_forward_cell(s, s_prime, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end))
              == O[l, t] for l in L for t in T), name = 'duration_start') #2d

# 2.4f
""" should we consider those move off the map next time period """
constrOltLessEqual = m.addConstrs((sum(X[l, s, s_prime, t] for s in S for s_prime in S_expand if is_forward_cell(s, s_prime, starting_c = s_init, ending_c = s_end, on_map_start = on_map_init, on_map_end = on_map_end)) 
...
# =============================================================================
    ...
    constr49b = m.addConstr(f_Z + sum([adj_finite_diff[l, s_c, t] * (ZZZ[l, s_c[0], t] - ZZZ_param[l, s_c[0], t]) for l in L for s_c in S_C for t in T if t not in T_no_detect and (s_c[0], t) not in part_T_no_detect_pairs]) <= Xi, name = 'cut_' + str(counter))
