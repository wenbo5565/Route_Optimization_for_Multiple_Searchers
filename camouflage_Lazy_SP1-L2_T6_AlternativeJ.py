...
# =============================================================================
    ...
    cut_constr = m.addConstrs((np.exp(-i * alpha) * (1 + i - i * np.exp(-alpha)) 
                  ...
                  <= U[omega_num] for omega_num in Omega_num for i in I), name = 'cut') #2d
    
    print('turn on lazy constraints')
    for i in I:
       if i < 3 or i > np.floor(1.3 * total_J) :
           for omega_num in Omega_num:
               # print('setting', omega, i, 'as lazy constraint')
               cut_constr[omega_num, i].Lazy = 3
